...
let hashtag = "#bytegrad";

const regex = new RegExp(`(?:^|\\s)${hashtag}(?=\\s|$|[,.!?])`, "i");
console.log(feedbacks[0].text);
console.log(regex.test(feedbacks[0].text));
...
// const hashtags = [
//   ...new Set(feedbacks.flatMap((feedback) => extractHashTags(feedback.text))),
// ];

// let existingHashTags = hashtags;

// const newHashTags = feedbacks.flatMap((feedback) =>
//   extractHashTags(feedback.text)
// ).filter((tag) => !existingHashTags.includes(tag));

// Not removed duplicates yet
// console.log("SET FLAT MAP: ", hashtags);

const company =
  "I love #coffee while coding #javascript in #starbucks"
    .split(" ")
